...
[MSL1,m,p]=picos(tmp_signal.*(tmp_signal>0),11);
[MSL2,m,p]=picos(tmp_signal.*(tmp_signal<0),7);

n=4;
filtro=ones(2*n+1,1); filtro=filtro/sum(filtro);
tmp_signal=conv(tibia_R,filtro);
tmp_signal=tmp_signal(1+n:end-n);
[MSR1,m,p]=picos(tmp_signal.*(tmp_signal>0),11);
[MSR2,m,p]=picos(tmp_signal.*(tmp_signal<0),7);
...
for k=1:length(MSL1) % para cada evento midstance detectado
    ...
    if (~isempty(k2))
        ...
        % entre k1 y k2
        [pmax,pmin,plocal]=picos(tibia_L(MSL1(k):MSL2(k2)),1);
        % Nos quedamos con los maximos que siguen al primer minimo
        pmax=pmax(find(pmax>pmin(1)));
        if (length(pmin)>0)
        ...
        if (length(pmin)>1)
            % El segundo mínimo es el siguiente candidato a HS
...
for k=1:length(MSR1)
    ...
    if (~isempty(k2))
        [pmax,pmin,plocal]=picos(tibia_R(MSR1(k)+5:MSR2(k2)),1);
        pmax=pmax(find(pmax>pmin(1)));
        if (length(pmin)>0)
...
for k=1:length(MSL2) % Para cada evento midstance de la fase de apoyo
    ...
    if (~isempty(k2))
        % si se encontro
        % se busca el evento TO como el mínimo absoluto entre ambos eventos
        % de midswing
...
if (nargin>2)
    ...
    hold
    plot(tibia_R,'g')
    plot(find(eventos(:,6)),tibia_R(find(eventos(:,6))),'m*');
    plot(find(eventos(:,5)),tibia_R(find(eventos(:,5))),'ms');
    plot(find(eventos(:,7)),tibia_R(find(eventos(:,7))),'md');
    plot(find(eventos(:,8)),tibia_R(find(eventos(:,8))),'mo');
    plot(find(eventos(:,2)),tibia_L(find(eventos(:,2))),'r*');
    plot(find(eventos(:,1)),tibia_L(find(eventos(:,1))),'rs');
    plot(find(eventos(:,3)),tibia_L(find(eventos(:,3))),'rd');
    plot(find(eventos(:,4)),tibia_L(find(eventos(:,4))),'ro');
end
...
prod=[];
for desp=1:n
    prod=[prod [datos((desp+1):end)-datos(1:(end-desp)); zeros(desp,1)].*[zeros(desp,1); datos((desp+1):end)-datos(1:(end-desp))]];
end
prod=all(prod<0,2);
p=find(prod);
