% ENTRENA_IDENT_ACT permite entrenar una red neuronal que identificará las actividades realizadas por un individuo.
%
% ENTRENA_IDENT_ACT Ajusta los parámetros de una red neuronal para que realice la identificación del movimiento realizado por
% un individuo, en base a un conjunto de datos patrón previamente capturados. La frecuencia de muestreo debe ser OBLIGATORIAMENTE de 
% 50Hz
%
% Sintax:
...
%    datos_bajarescaleras  - variable tipo cell array dentro de la cuál cada uno
...
%    datos_bajarrampa      - variable tipo cell array dentro de la cuál cada uno
%                           de los elementos es una matriz con los datos correspondientes a
%                           un tramo en el que el individuo se encontraba bajando una rampa.
%    datos_andar           - variable tipo cell array dentro de la cuál cada uno
%                           de los elementos es una matriz con los datos correspondientes a
%                           un tramo en el que el individuo se encontraba andando.
%    datos_subirrampa      - variable tipo cell array dentro de la cuál cada uno
%                           de los elementos es una matriz con los datos correspondientes a
%                           un tramo en el que el individuo se encontraba subiendo una rampa.
%    datos_subirescaleras  - variable tipo cell array dentro de la cuál cada uno
%                           de los elementos es una matriz con los datos correspondientes a 
...
%   		1: El tiempo
%   		2: La aceleración en el eje x
%	   	3: La aceleración en el eje y
%   		4: La aceleración en el eje z
%   		5: Giróscopo en el eje x (opcional)
%   		6: Giróscopo en el eje y (opcional)
%   		7: Giróscopo en el eje z (opcional)
%   		8: Campo magnético en el eje x (opcional)
%   		9: Campo magnético en el eje y (opcional)
%   		10: Campo magnético en el eje z (opcional)
%
%
% Parámetros de salida:
%    redneuronal   - Variable que tiene almacenada la red neuronal
%    parametros    - Variable con datos genericos sobre la señal, necesarios para ident_act.
%
% Examples:
% 
% See also: ident_act
...
%           28.12.2007  Preparación para la toolbox
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Llamada al programa que tratará los datos almacenados en el fichero
...
[datos_c,RR]=ejes_anatomicos(datos1,datos2,R);
...
for indice=1:iteraciones
    ...
    %escaleras se tomarán datos desde la muestra en t=0,64 segundos hasta
    %la muestra en t=tiempototal-0,64 segundos.
    %En el resto de los casos las muestras con las que se entrenará la red
    %neuronal se tomarán desde t=1,28 segundos hasta
    ...
    %Se considera la primera muestra errónea y por eso no se contabilizará

    while i <= longitudmedida+1-limite
        %La última muestra también se considerará errónea y por eso tampoco
        %se tomará
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %Llamada a subrutina que se encargará de tomar los datos principales
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        [energiawavelety,desvstdwavelety,valorRMSwavelety]=energiawavelet(acelybucle);
        ...
        %El valorRMSwavelety no se usará ya que esos valores en esa
        %dirección del espacio (medio-lateral) resultan inútiles.
        ...
        if accion==1 vectoraccion=[1 -1 -1 -1 -1]; end
        if accion==2 vectoraccion=[-1 1 -1 -1 -1]; end
        if accion==3 vectoraccion=[-1 -1 1 -1 -1]; end
        if accion==4 vectoraccion=[-1 -1 -1 1 -1]; end
        if accion==5 vectoraccion=[-1 -1 -1 -1 1]; end

        matrizresultadoacciones=[matrizresultadoacciones; vectoraccion];
...
%puesto que ya se encuentra entre los valores -1 y 1 (ya está normalizado)
[matrizresultadoaccionesn,minmatrizresultadoacciones,maxmatrizresultadoacciones]=premnmx(matrizresultadoacciones);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Creación de la matriz con los mínimos y los máximos
% matrizminmax=[(min(mtotalnor'))' (max(mtotalnor'))'];

matrizminmax=[(min(mtotalnor'*1.5))' (max(mtotalnor'*1.5))'];

%Vector con el número de neuronas: 10 en la 1ª y 2ª capas y 5 en la 3ª
nneuronas=[10 10 5];
...
redneuronal.trainParam.epochs=8000;      %Se realizarán 1000 pasadas
redneuronal.trainParam.show=25;          %Cada 25 pasadas se realizará un display
%redneuronal.trainParam.goal=1e-5;       %Precisión del objetivo
%redneuronal.trainParam.min_grad=0;      %Gradiente mínimo
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Se guardará la variable redneuronal en el fichero red_XXXX.mat, siendo
...
%Gracias a esta forma ya se tendrá salvada la variable para su utilización
%en otros programas.
...
%    maxmtotalnor  - Un vector con los máximos que poseían las variables
%                    antes de ser normalizadas.
%    minmtotalnor  - Un vector con los mínimos que poseían las variables
%                    antes de ser normalizadas.
%    valormediox   - El valor medio de la señal de aceleración en el eje x
%    valormedioy   - El valor medio de la señal de aceleración en el eje y
%    valormedioz   - El valor medio de la señal de aceleración en el eje z
%    datos1        - Valores necesarios para la corrección de los ejes
%                    anatómicos.
...
%acelxbucle->Vector de aceleración en el eje x en el intervalo que se 
%quiere medir
%acelybucle->Vector de aceleración en el eje y en el intervalo que se 
%quiere medir
%acelzbucle->Vector de aceleración en el eje z en el intervalo que se 
%quiere medir
...
%Función anidada
%toma de una matriz los valores del tiempo y de tres ejes de aceleración
...
%aceleracionx->Vector de aceleración en el eje x
%aceleraciony->Vector de aceleración en el eje y
%aceleracionz->Vector de aceleración en el eje z
...
%Toma de datos
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Se mira el número de filas que poseía el fichero original
