...
function CONFIG = connectsilop(CONFIG, modo_simulacion, log, bps, freq, modo, buffer)
    ...
    global t
        ...
        % Actualizar los valores de las se馻les
        switch (xbus.modo)
            case 0,
                factor=9;
            case 1,
        ...
        if (CONFIG.SENHALES.TIBIA_DCHA.Serie~=-1)
            ...
            if (isempty(p))
                error('SilopToolbox:connectsilop','El n鷐ero de serie del sensor asignado al TIBIA DCHA no ha sido encontrado');
            else
        ...
        if (CONFIG.SENHALES.TIBIA_IZDA.Serie~=-1)
            ...
            if (isempty(p))
                error('SilopToolbox:connectsilop','El n鷐ero de serie del sensor asignado al TIBIA IZDA no ha sido encontrado');
            else
        ...
        if (CONFIG.SENHALES.PIE_DCHO.Serie~=-1)
            ...
            if (isempty(p))
                error('SilopToolbox:connectsilop','El n鷐ero de serie del sensor asignado al PIE DCHO no ha sido encontrado');
            else
        ...
        if (CONFIG.SENHALES.PIE_IZDO.Serie~=-1)
            ...
            if (isempty(p))
                error('SilopToolbox:connectsilop','El n鷐ero de serie del sensor asignado al PIE IZDO no ha sido encontrado');
            else
