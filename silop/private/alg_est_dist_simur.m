%ALG_EST_DIST_SIMUR Algoritmo para la estimaci�n de la longitud de los pasos mediante el modelo pendulo+desplazamiento
%
%ALG_EST_DIST_SIMUR Algoritmo para la estimaci�n de la longitud de los pasos.Este algoritmo actua como 
%wrapper de la funci�n distancia_penduloparcial
%Se puede configurar mediante:
%    Config = addalgoritmo(Config, 'alg_est_dist_simur', 1, [COG.Acc_Z], [freq,hsensor,pie], {'alg_det_event'});
%
		...
		if (length(eventos_hs)>1)
			...
			if (isnan(resultado(k)))
				if ((k-viejoevento)<200) %Menos de dos segundos un paso
					...
					if (isempty(to))
						warning('Toe-Off no detectado. Se pierde un paso');
					else
